...
- **Feedback** â€” confirming the interface heard the user
- **Spatial consistency** â€” showing where something came from or went
- **State indication** â€” making a state change legible
- **Preventing a jarring change** â€” bridging content that would otherwise teleport
- **Explanation** â€” demonstrating how something works (marketing/onboarding only)
- **Delight** â€” allowed *only* at the rare/first-time tier
...
### 3. Pick the tool â€” cheapest that works
...
CSS animations beat JS under load â€” they run off the main thread, while `requestAnimationFrame`-based animation drops frames while the browser loads, scripts, or paints. Use CSS for predetermined motion, JS for dynamic and interruptible motion.

If the task needs a *component* rather than an animation â€” a toast, a drawer, a command menu, a dropdown â€” stop and invoke `pick-ui-library`. Hand-rolling those is how you end up with a `<div>` dropdown and no focus management.

### 4. Pick the properties

- **`transform` and `opacity` only.** They skip layout and paint and run on the GPU. `width`/`height`/`margin`/`padding`/`top`/`left` trigger all three. (`clip-path` is the sanctioned fourth â€” see RECIPES.md. `height` is tolerated only for accordions, where there's no transform equivalent.)
- **Never `scale(0)`.** Start from `scale(0.9â€“0.97)` + `opacity: 0`. Nothing in the real world appears from nothing.
- **`transform-origin` at the trigger** for popovers, dropdowns, menus, tooltips â€” `var(--transform-origin)` in Base UI. **Modals are exempt**; they're not anchored to a trigger, so they stay centered.
- **Percentages in `translate()`** are relative to the element's own size â€” `translateY(100%)` moves by its own height whatever the content. Prefer over hardcoded pixels.
...
- **Never drive a child's transform from a CSS variable on the parent** â€” it recalculates styles for every child. Set `transform` on the element directly.

### 5. Easing and duration â€” or a spring
...
```js
{ type: "spring", duration: 0.5, bounce: 0.2 }        // Apple-style â€” easier to reason about
{ type: "spring", mass: 1, stiffness: 100, damping: 10 }  // traditional physics â€” more control
```

Keep bounce at 0.1â€“0.3, and avoid bounce in most UI â€” reserve it for drag-to-dismiss and playful interactions.

### 6. Interruption and exit

- **Transitions, not keyframes, for anything triggered rapidly** â€” toasts, toggles, anything a user can fire twice in a second. Transitions retarget from the current value; keyframes restart from zero.
...
Reduced motion means **fewer and gentler** animations, not zero â€” keep transitions that aid comprehension, remove movement and position changes.

## Recipes

For ready-to-build implementations of the common cases â€” button press, dropdown, tooltip, modal, drawer, toast, accordion, stagger, hold-to-confirm, tab indicator, scroll reveal, drag-to-dismiss â€” see [RECIPES.md](RECIPES.md). Load it whenever the request matches one of those components; start from the recipe rather than from a blank file.
...
| UI duration over 300ms with no reason | 150â€“250ms |
...
| Everything entering at once | 30â€“80ms stagger |

## Output

Write the code. Then, in at most a few lines:

- **The gate result** â€” frequency tier and the named purpose. If something in the request was rejected, say which and why.
- **The ingredients** â€” tool, properties, curve, duration or spring config, in one line each.
- **What to feel-check** â€” if the result depends on feel you can't judge from code (a crossfade, a spring's bounce, the opacity/height balance in an entering list), say so and point at the check: play it at 2â€“5Ã— duration or in the DevTools animation inspector, step it frame by frame, test gestures on a real device, and look again the next day with fresh eyes.

Don't pad this into a report. The code is the deliverable.

## Tone

Opinionated and brief. When the honest answer is "this shouldn't animate," give it â€” that answer is the reason this skill exists. When feel genuinely can't be settled from code, say so instead of guessing at a value.
